Here is a revised and unified version of your master's thesis data documentation:
...
| `level`       | Integer | Classification depth of the NACE code: \<ul\>\<li\>`0` = Top-level aggregate (e.g., `"B+C+D+E"`)\</li\>\<li\>`1` = One-letter division (e.g., `"A"`, `"B"`)\</li\>\<li\>`2` = Two-digit subdivision (e.g., `"49"`, `"58"`)\</li\>\</ul\> |
...
| `metric`      | String  | Identifier for the indicator. Possible values: \<ul\>\<li\>`ppi_by_nace_industry`: Industrial PPI (NACE divisions B–E; from CZSO “industry” sheet).\</li\>\<li\>`ppi_by_nace_aggregated`: SPPI for non‐industry sections (rebased to 2015 = 100).\</li\>\<li\>`ppi_by_nace`: Combined PPI series (concatenation of the two above).\</li\>\<li\>`avg_wages_by_nace`: Average gross monthly wage in CZK (per full‐time equivalent).\</li\>\<li\>`no_of_employees_by_nace`: Number of employees (in thousands).\</li\>\</ul\> |
| `value`       | Float   | Numeric value of the `metric`. Missing values are represented as `NaN`.                                                                                                   |
| `unit`        | String  | Unit of measure for the `value`. Possible values: \<ul\>\<li\>`"2015=100"` (for all PPI metrics)\</li\>\<li\>`"CZK_avg_gross_monthly_per_fulltime"` (for wages)\</li\>\<li\>`"ths"` (for number of employees)\</li\>\</ul\> |
| `source`      | String  | Indicates the CZSO subset from which the data row originated. Possible values: \<ul\>\<li\>`"CZSO – industry"` (for industrial PPI)\</li\>\<li\>`"CZSO – without industry"` (for aggregated SPPI)\</li\>\<li\>`"CZSO"` (for wages and employees)\</li\>\</ul\> |
